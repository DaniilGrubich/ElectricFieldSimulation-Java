...
public class Main extends JFrame{
    ...
    Main(){
        //configure frame
        super("Electric Field Simulation By Daniil Pavlovich Grubich");
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        setSize(2000*2/3, 1000*2/3);
        setLayout(new BorderLayout()); getContentPane().setBackground(new Color(50, 50, 50));

        //create frame components
        Canvas c = new Canvas();
        add(c, BorderLayout.CENTER);
        c.addMouseListener(new CanvasMouseCallbacks());
        c.addKeyListener(new CanvasKeyCallbacks());

        int nRows = 10;
        controlsPanel = new JPanel(new GridLayout(nRows, 1,10, 10));
        controlsPanel.setBorder(new EmptyBorder(10, 10, 10, 10));
        controlsPanel.setBackground(Color.lightGray); controlsPanel.setPreferredSize(new Dimension(300, 1));

        //add charge amplification slider
        JPanel chargeAmplificationPanel = new JPanel(new GridLayout(2, 1, 0,0));

        JLabel chargeGainLabel = new JLabel("Stationary Charge Amplification\t x10^0");
        JSlider chargeGainSlider = new JSlider(JSlider.HORIZONTAL, -5, 5, 0);
        chargeGainSlider.setPaintLabels(true);
        chargeGainSlider.addChangeListener(e -> {
            ParticleSystem.nonStationaryChargeMuliplyer = (float) Math.pow(10, chargeGainSlider.getValue());
            chargeGainLabel.setText("Stationary Charge Amplification \t x10^" + chargeGainSlider.getValue());
        });

        chargeAmplificationPanel.add(chargeGainLabel);
        chargeAmplificationPanel.add(chargeGainSlider);
        controlsPanel.add(chargeAmplificationPanel);
        ///

        //add tail length slider
        JPanel tailLengthPanel = new JPanel(new GridLayout(2, 1, 0,0));

        JLabel tailLengthLabel = new JLabel("Charge Tail Length\t 1");
        JSlider tailLengthSlider = new JSlider(JSlider.HORIZONTAL, 1, 30, 1);
        tailLengthSlider.setPaintLabels(true);
        tailLengthSlider.addChangeListener(e -> {
            ParticleSystem.setTailLength(tailLengthSlider.getValue());
            tailLengthLabel.setText("Charge Tail Length\t " + tailLengthSlider.getValue());
        });

        tailLengthPanel.add(tailLengthLabel);
        tailLengthPanel.add(tailLengthSlider);
        controlsPanel.add(tailLengthPanel);
        ///


        for(int i = 2; i < nRows; i++) {
            JButton placeholderLabel = new JButton("Placeholder #" + i);
            placeholderLabel.setBackground(Color.gray);
            controlsPanel.add(placeholderLabel);
        }
        ...
        new Timer().scheduleAtFixedRate(new GlobalTask(c), 1000/60, 1000/60);
    }
}
